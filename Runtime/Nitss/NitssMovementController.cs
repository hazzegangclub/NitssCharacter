...
namespace Hazze.Gameplay.Characters.Nitss
{
    ...
    public sealed class NitssMovementController : MonoBehaviour
    {
        ...
        private NitssJumpAttackModule jumpAttackModule;
        ...
        private const float DefaultStationaryYawLeft = 230f;
        ...
        private void Awake()
        {
            ...
            else
            {
                ...
                // Remove fricção entre personagens
                body.collisionDetectionMode = CollisionDetectionMode.Continuous;
                
                // Cria ou usa physics material sem fricção
                var colliders = GetComponentsInChildren<Collider>();
                foreach (var col in colliders)
                {
                    if (col.sharedMaterial == null || col.sharedMaterial.name == "Default (Instance)")
                    {
                        var noFrictionMat = new PhysicsMaterial("NoFriction")
                        {
                            dynamicFriction = 0f,
                            staticFriction = 0f,
                            bounciness = 0f,
        ...
        private void RotateVisual(float dt, bool stationary)
        {
            ...
            bool attacking = (combatController != null && combatController.IsAttacking) || 
                           (jumpAttackModule != null && jumpAttackModule.IsAirComboActive);
            float targetYaw;
            if (attacking)
            {
        ...
        public void BumpVerticalClamped(float minVelocityY, float maxHeight)
        {
            if (!body) return;
            
            // Calcula a velocidade necessária para atingir a altura máxima
            // v² = 2 * g * h
            float gravity = Mathf.Abs(Physics.gravity.y);
            float maxVelocity = Mathf.Sqrt(2f * gravity * maxHeight);
            
            // Limita a velocidade ao máximo calculado
            float clampedVelocity = Mathf.Min(minVelocityY, maxVelocity);
            
            Vector3 v = body.linearVelocity;
            if (v.y < clampedVelocity)
            {
                v.y = clampedVelocity;
                body.linearVelocity = v;
            }
